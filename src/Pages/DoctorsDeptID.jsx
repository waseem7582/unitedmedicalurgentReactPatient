...
export default function DoctorsByDeptID({ deptID, deptName }) {
  const getData = async () => {
    const res = await GET(`get_doctor_dep_id/${deptID}`);
    return res.data;
  };
        ...
        {!data.length ? (
          <>
            ...
            >
              ...
              <Text display={"inline-block"} fontWeight={600} color={"primary.text"}>{deptName}</Text> department.
              Please check back later or consider a different department. If you
              require immediate assistance, our support team is here to help you
              with any urgent concerns. Thank you for your understanding and
              patience.
            </Text>
          </>
        ) : null}
